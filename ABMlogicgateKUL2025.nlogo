...
Signals & logic: patches track fields for elicitin, ROS, and glucan fragments. Turtles (bacteria) read local elicitin and ROS and compute Hill responses; the intracellular AND combines them. With leak and gain, mRNA (m) and protein (y) follow first-order dynamics. A bacterium turns ON when y ≥ y-threshold (with a small probabilistic bias toward the infection center).
Secretion (two-phase):
Baseline containment: low AMP background exists when no pathogen is present via periodic randomized activity.
Invasion boost: if pathogen is present and a cell is ON, it secretes q + q_boost, distance-weighted to focus secretion near the infection.
Transport & environment: dehydration (0–1) jointly controls diffusion-rate and evaporation-rate (computed from min/max bounds) and attenuates transcriptional gain (proxy for stress). Wet = broader, persistent AMP halos; dry = tighter, quickly cleared halos.
Pathogen dynamics: the Phytophthora “blob” is a noisy disk with user-set radius/biomass. It decays naturally and is killed sigmoidally by AMP (MIC-like scale; capped fractional kill per tick). Elicitin is produced by pathogen; glucan is released where AMP meets pathogen; ROS accumulates from AMP contact, pathogen stress, and glucan amplification, then decays.
Protection metric: the model tracks the protected area (patches with AMP >= C_eff) and records time-to-protection when a target area is first achieved.
Kill switch (optional): a global toxin can be injected; turtles on patches with toxin >= lethal-dose die immediately.

## HOW TO USE IT

SETUP initializes bacteria, signal fields, and a noisy Phytophthora blob centered in the world.
GO runs the simulation.
Key controls:
POPULATION: number of bacteria created at setup.
PHYTO-SIZE: scales blob radius and initial biomass.
DEHYDRATION (0–1): sets environmental stress and implicitly sets diffusion-rate and evaporation-rate each tick.
SECRETION?: toggle secretion logic.
PLOT?: toggle plotting.
KILL-SWITCH button: injects toxin-dose everywhere; turtles die if local toxin >= lethal-dose

## THINGS TO NOTICE

Focused response: AMP halos concentrate near the infection center due to distance-weighted boosting and probabilistic ON bias; pathogen-free regions remain mostly quiet.
Moisture shaping: lower dehydration (moisture) broadens and sustains halos; higher dehydration tightens and clears them—yet the AND gate maintains specificity across moderate stress.
Switch behavior: activation requires both cues; single-input stimulation should not trigger strong expression. After cues drop, bacteria relax back below threshold.


## EXTENDING THE MODEL

Logic variants: replace AND (H1*H2) with NAND or extend to 3-input logic; explore how false positives/negatives change.
Population ecology: add bacterial and pathogen replication, resource limits, or taxis.
Heterogeneity: give cells diverse thresholds (y-threshold), K values, or secretion rates.
Environment: decouple diffusion/evaporation sliders from dehydration; add soil structure,etc.
Sensing:modify Hill exponents (n1, n2) or sensitivities (K1, K2); explore robustness plateaus and failure modes.
Defense repertoire: vary C_eff, q, q_boost, or the distance fall-off to tune focusing.

## NETLOGO FEATURES

Uses diffuse for AMP transport and simple per-tick evaporation.
Employs patch-local state variables for signals (elicitin, ROS, glucan) and pathogen biomass.
Demonstrates probabilistic decisions and ODE-like updates in agent procedures with time-step.
